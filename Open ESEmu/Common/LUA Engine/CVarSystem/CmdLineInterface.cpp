...
std::size_t CmdLineInterface::GetCommandIndex( std::string strCommand )
{
	if( strCommand.empty( ) )
	{
		LogWarning( "CmdLineInterface::GetCommandIndex: empty command!" );
		return -1;
	}
...
bool CmdLineInterface::GetCommand( std::string strCommand, CmdCommand_t *pCommand )
{
	if( strCommand.empty( ) )
	{
		LogWarning( "CmdLineInterface::GetCommand: empty command!" );
		return false;
	}
	...
	if( nIndex == -1 )
	{
		LogWarning( "CmdLineInterface::GetCommand: %s isn't registered!", strCommand.c_str( ) );
		return false;
	}
...
bool CmdLineInterface::GetCommandsFromCmdLine( int iArgC, char *pszArg[] )
{
	...
	{
		LogWarning( "CmdLineInterface: missing arguemts or parameters!" );
		return false;
	}
...
bool CmdLineInterface::is_command( std::string strCommand )
{
	if( strCommand.empty( ) )
	{
		LogWarning( "CmdLineInterface::is_command: empty command!" );
		return false;
	}
...
void CmdLineInterface::SetCommandValue( std::string strCommand, std::string strValue )
{
	if( strCommand.empty( ) )
	{
		LogWarning( "CmdLineInterface::SetCommandValue: empty command!" );
		return;
	}
	...
	if( nIndex == -1 )
	{
		LogWarning( "CmdLineInterface::SetCommand: %s isn't registered!", strCommand.c_str( ) );
		return;
	}
...
void CmdLineInterface::RegisterCommand( std::string strCommand )
{
	if( strCommand.empty( ) )
	{
		LogWarning( "CmdLineInterface::RegisterCommand: empty command!" );
		return;
	}

	if( this->GetCommandIndex( strCommand ) != -1 )
	{
		LogWarning( "CmdLineInterface:RegisterCommand: %s is already registered!", strCommand.c_str( ) );
		return;
	}
